...
import { fetchJobStories, fetchNews } from "../api/apicalls";
import { contextNews } from "../context/context";
import { FETCH_JOBNEWS } from "../context/action.type";
...
const Jobs = () => {
  ...
  const { jobsStories, jobsStoriesNews } = newsContext.state;

  useEffect(() => {
    fetchJobStories(dispatch);
  }, []);

  useEffect(() => {
    jobsStories.map((newsid, i) => {
      if (i + newsStartCount < newsCount) {
        fetchNews(dispatch, newsid, FETCH_JOBNEWS);
      }
    });
  }, [jobsStories, newsCount]);

  return (
    <div className="wrapper">
      <div className="container">
        <Navbar />
        {jobsStoriesNews.map((news, i) => {
